  ...
  return (
    <section className="rounded-2xl border border-amber-500/30 bg-amber-500/5 p-6">
      ...
      <p className="mt-3 text-center text-[13px] leading-relaxed text-body">
        ...
        <br />
        {displayName}님의 재물·관계·일·실천 전략 전체를 확인해보세요.
        <br />
        결제 후 잠긴 {totalLocked}개 항목이 즉시 해금됩니다.
      </p>
      ...
      <div className="mt-3 grid grid-cols-1 gap-1.5">
        <Link
          href={`/products/${bestSlug}`}
          onClick={() => trackEvent("unlock_cta_click", { page: "free_result", position: "paywall_secondary" })}
          className="text-center text-[12px] text-body hover:text-ink underline underline-offset-4"
        >
          잠긴 {totalLocked}개 항목 전체 해금하기
        </Link>
        <Link
          href="/products/life-master"
          onClick={() => trackEvent("unlock_cta_click", { page: "free_result", position: "paywall_tertiary" })}
          className="text-center text-[12px] text-body hover:text-ink underline underline-offset-4"
        >
          {cta.tertiary}
        </Link>
